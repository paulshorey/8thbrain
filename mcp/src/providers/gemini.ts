 ...
 * Exposes run_deep_research tool using Google Gemini Deep Research agent.
 */
      ...
      while (Date.now() - start < maxWaitMs) {
        ...
        while (retries <= MAX_POLL_RETRIES) {
          try {
            ...
            if (retries > MAX_POLL_RETRIES) {
              ...
              return {
                content: [
                  {
                    type: "text",
                    text: `Failed to poll research status after ${MAX_POLL_RETRIES} retries: ${err}`,
                  },
                ],
              };
            }
            ...
            log(
              "info",
              `Poll failed (attempt ${retries}/${MAX_POLL_RETRIES}), retrying in ${delayMs}ms: ${err}`
            );
            await sleep(delayMs);
          }
        }
