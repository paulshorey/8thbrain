...
function apiKeyMiddleware(req: Request, res: Response, next: () => void): void {
  const expected = process.env.MCP_KEY;
  if (!expected) {
    console.error("MCP_KEY not set; rejecting all requests. Set MCP_KEY for HTTP mode.");
    res.status(500).send("Server misconfigured: MCP_KEY required");
    return;
  }
