...
- The server runs in **HTTP mode** when `MCP_TRANSPORT=http` (or `streamable-http`). It listens on `MCP_HOST` / `MCP_PORT` and exposes:
  - **Gemini Deep Research:** `POST/GET/DELETE` at `/gemini/mcp`
  - **Perplexity Sonar:** `POST/GET/DELETE` at `/perplexity/mcp`
- Clients use the **Streamable HTTP** transport: one URL per provider, with optional `X-API-Key` (or other) headers for auth.
- TLS (HTTPS) is **not** implemented inside the app. Use a reverse proxy (e.g. Nginx, Caddy) on the VPS to terminate HTTPS and proxy to this server.
...
- **HTTP mode:** `MCP_API_KEY` (secret for `X-API-Key` header)
...
2. **Environment:** Create a `.env` (or use systemd/process manager env) with:
   ...
   - `PERPLEXITY_API_KEY`
   - `MCP_API_KEY` (strong random secret for remote clients)
   - Optionally: `MCP_HOST=127.0.0.1`, `MCP_PORT=8000` (so only the reverse proxy talks to the app).
3. **Run the server** (HTTP mode):
   ```bash
   MCP_TRANSPORT=http npm start
   ```
   Or with env from file:
   ```bash
   set -a && source .env && set +a && MCP_TRANSPORT=http node --import tsx src/index.ts
   ```
...
- **Auth:** All requests to `/gemini/mcp` and `/perplexity/mcp` require the `X-API-Key` header to match `MCP_API_KEY`. Use a long, random value and keep it secret.
- **Firewall:** Allow 80/443 on the VPS; do not expose the Node port (e.g. 8000) publicly unless you add auth at the proxy.
- **DNS rebinding:** With `MCP_HOST=0.0.0.0`, the SDK may warn about missing DNS rebinding protection; behind a reverse proxy and with `MCP_API_KEY`, this is acceptable. For stricter setups, bind to `127.0.0.1` and proxy only.

## Health / sanity check

From your machine (with `MCP_API_KEY` set locally):
